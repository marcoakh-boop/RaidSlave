-- RaidSlaveInvite.lua - Auto-invite/role assign/gearcheck for "vanilla"-compliant versions of Wow
-- Created by Doite
...
INV._sessionIgnores = {}
function RaidSlaveInvite.ResetSessionIgnores()
...
local function RB() return RaidSlaveRaidBuilder end
...
local function setRole(name, role)
  ensureRoleBuckets()
  clearAllRoles(name)
  if role == "TANK" then RaidSlaveDB.Tanks[name] = true
  elseif role == "HEALER" then RaidSlaveDB.Healers[name] = true
  elseif role == "DPS" then RaidSlaveDB.DPS[name] = true
  end
  ...
  local function GetCounts()
    ...
    for name in pairs(present) do
      if not exclude[name] then
        ...
        if RaidSlaveDB.Tanks[name] then
          cT = cT + 1
        elseif RaidSlaveDB.Healers[name] then
          cH = cH + 1
        elseif RaidSlaveDB.DPS[name] then
          cD = cD + 1
        end
      end
    end
...
function INV.ShowConfirm(name, role, classHint, offeredLetters, rbMode)
  if not INV._confirm then
    local p = CreateFrame("Frame", "RaidSlaveInviteConfirm", UIParent)
    INV._confirm = p
...
INV._evt:SetScript("OnEvent", function()
  ...
  local S = RaidSlaveDB and RaidSlaveDB.Builder
...
function INV.Open()
  ...
  local f = CreateFrame("Frame", "RaidSlaveInviteFrame", UIParent)
  INV.frame = f
  ...
  local e = CreateFrame("EditBox", "RaidSlaveInviteKeyword", f, "InputBoxTemplate")
  INV.ui.edit = e
  ...
  local cb = CreateFrame("CheckButton", "RaidSlaveInviteAutoAssign", f, "UICheckButtonTemplate")
  INV.ui.cb = cb
  INV.autoAssign = false
  cb:SetPoint("TOPLEFT", lbl, "BOTTOMLEFT", 0, -5)
  cb:SetWidth(20); cb:SetHeight(20)
  getglobal("RaidSlaveInviteAutoAssignText"):SetText("Auto-assign roles")
  ...
  off:SetText("Hide")
	off:SetScript("OnClick", function()
	  f:Hide()
	  if INV.enabled then
		cfmsg("Auto-Invite (standalone) is still running in the background. Use /rsai or /rs autoinvite to reopen.")
	  end
	end)
...
-- slash
SLASH_TTACTINV1 = "/rsai"
SLASH_TTACTINV2 = "/ttautoinvite"
SlashCmdList["TTACTINV"] = function() RaidSlaveInvite.Open() end
