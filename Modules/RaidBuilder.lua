-- RaidSlaveRaidBuilder.lua - Raid/LFM Builder with Auto-Announcer for "vanilla"-compliant versions of Wow
-- Created by Doite
...
-- Module & saved state
-------------------------------------------------
local RB = RaidSlaveRaidBuilder or {}
RaidSlaveRaidBuilder = RB

local function EnsureDB()
  if not RaidSlaveDB then RaidSlaveDB = {} end
  if not RaidSlaveDB.Builder then RaidSlaveDB.Builder = {} end
  if not RaidSlaveDB.BuilderFrame then RaidSlaveDB.BuilderFrame = {} end
...
local function Saved() EnsureDB(); return RaidSlaveDB.Builder end
...
local function PresetsDB()
  EnsureDB()
  if not RaidSlaveDB.BuilderPresets then RaidSlaveDB.BuilderPresets = {} end
  return RaidSlaveDB.BuilderPresets
end
...
local function RB_CountUtility(utilKey)
  ...
  for i=1,n do
    ...
    if name and classLoc and classset[classLoc] then
      local isTank = RaidSlaveDB and RaidSlaveDB.Tanks and RaidSlaveDB.Tanks[name] == true
...
local function ShortRaidLabel(full)
  if not full or not RaidSlave or not RaidSlave.Aliases then return full end
  local nicify = { ony="Ony", kara10="Kara10", kara40="Kara40", world="World", es="ES" }
  for short, long in pairs(RaidSlave.Aliases) do
...
-- Public notifier: call from the Roles module after updating RaidSlaveDB.Tanks/Healers/DPS
function RaidSlaveRaidBuilder.NotifyRoleAssignmentChanged()
  RB._dirty = true
  RB.RefreshPreview()
end

function RaidSlaveRaidBuilder.AutoRolesEnabled()
...
local function SaveFramePosition()
  ...
  RaidSlaveDB.BuilderFrame.position = {
    point = point, relativeTo = "UIParent", relativePoint = relativePoint, x = x, y = y,
  }
  RaidSlaveDB.BuilderFrame.locked = RB.frame.locked and true or false
end
local function ApplyLockIcon()
  if not RB.lockButton then return end
  if RB.frame and RB.frame.locked then
    RB.lockButton:SetNormalTexture("Interface\\AddOns\\RaidSlave\\Media\\raidslave-lock")
  else
    RB.lockButton:SetNormalTexture("Interface\\AddOns\\RaidSlave\\Media\\raidslave-unlock")
  end
end
local function RestoreFramePosition()
  if not RB.frame then return end
  EnsureDB()
  local st = RaidSlaveDB.BuilderFrame or {}
...
-------------------------------------------------
-- Auto-Invite wiring (RB -> RaidSlaveInvite)
-------------------------------------------------
...
local function RB_AutoInviteUpdateFromUI(source)
  ...
  if RaidSlaveInvite and RaidSlaveInvite.SetFromRB then
    RaidSlaveInvite.SetFromRB(enabled, "", autoRoles)
  end
...
local function RB_SyncInviteExtras()
  if RaidSlaveInvite and RaidSlaveInvite.SetGearcheckFromRB then
    RaidSlaveInvite.SetGearcheckFromRB(RB.state.autoGear and true or false, RB.state.gearScale)
  end
end
...
function RB.InitRaidDropdown()
  UIDropDownMenu_Initialize(RB.ddRaid, function()
    local raids = {}
    local rn
    for rn,_ in pairs(RaidSlave and RaidSlave.DefaultData or {}) do table.insert(raids, rn) end
    table.sort(raids)
...
function RB.InitWBossDropdown()
  UIDropDownMenu_Initialize(RB.ddWBoss, function()
    local bosses, wb = {}, RaidSlave and RaidSlave.DefaultData and RaidSlave.DefaultData["World Bosses"]
    if wb then
      local bossName
...
local function ShowAutoConfirm()
  ...
  local wf = CreateFrame("Frame", "RaidSlaveRBAutoConfirm", parent)
  RB._confirm = wf
  ...
  b:SetText("You have selected to auto-announce. Therefore, you can not close the RaidSlave Raid Builder window, until you stop auto-announcing. USE RESPECTFULLY!")
...
-------------------------------------------------
function RaidSlaveRaidBuilder.AnnounceOnce()
  local RBm = RaidSlaveRaidBuilder
...
function RB.Open()
  ...
  local f = CreateFrame("Frame", "RaidSlaveRaidBuilderFrame", UIParent)
  RB.frame = f
  ...
  f:SetScript("OnHide", function()
    RB.state.running=false; RB._warnOk=false; RB._nextSend=nil; RB.UpdateButtonsForRunning()
    if RaidSlaveInvite and RaidSlaveInvite.ResetSessionIgnores then RaidSlaveInvite.ResetSessionIgnores() end
    RB_SyncInviteExtras()
  end)

  local title = f:CreateFontString(nil, "OVERLAY", "GameFontNormal")
  title:SetPoint("TOPLEFT", f, "TOPLEFT", 20, -14)
  title:SetText("|cffff6600RaidSlave Raid Builder|r")
  ...
  RB.lockButton = CreateFrame("Button", "RaidSlaveRBlock", f)
  ...
  RB.editPresetName = CreateFrame("EditBox", "RaidSlaveRBPresetName", f, "InputBoxTemplate")
  ...
  RB.ddPresetLoad = CreateFrame("Frame", "RaidSlaveRBPresetLoad", f, "UIDropDownMenuTemplate")
  RB.ddPresetLoad:SetPoint("LEFT", RB.editPresetName, "RIGHT", -13, -3)
  UIDropDownMenu_SetWidth(70, RB.ddPresetLoad)

  RB.ddPresetRemove = CreateFrame("Frame", "RaidSlaveRBPresetRemove", f, "UIDropDownMenuTemplate")
  ...
  RB.ddRaid = CreateFrame("Frame", "RaidSlaveRBRaid", f, "UIDropDownMenuTemplate")
  RB.ddRaid:SetPoint("TOPLEFT", f, "TOPLEFT", 70, -95); RB.ddRaid:SetWidth(180)
  
  RB.ddWBoss  = CreateFrame("Frame", "RaidSlaveRBWorldBoss", f, "UIDropDownMenuTemplate")
  RB.ddWBoss:SetPoint("LEFT", RB.ddRaid, "RIGHT", -28, 0); RB.ddWBoss:SetWidth(160)
  
  RB.ddESMode = CreateFrame("Frame", "RaidSlaveRBESMode", f, "UIDropDownMenuTemplate")
  RB.ddESMode:SetPoint("LEFT", RB.ddRaid, "RIGHT", -28, 0); RB.ddESMode:SetWidth(160)

  RB.editCustomRaid = CreateFrame("EditBox", "RaidSlaveRBCustomRaidText", f, "InputBoxTemplate")
  ...
  RB.ddSize = CreateFrame("Frame", "RaidSlaveRBSize", f, "UIDropDownMenuTemplate")

  RB.editCustomSize = CreateFrame("EditBox", "RaidSlaveRBCustomSize", f, "InputBoxTemplate")
  ...
  RB.ddTanks = CreateFrame("Frame", "RaidSlaveRBTanks", f, "UIDropDownMenuTemplate")
  ...
  RB.ddHealers = CreateFrame("Frame", "RaidSlaveRBHealers", f, "UIDropDownMenuTemplate")
  ...
  RB.ddSRs = CreateFrame("Frame", "RaidSlaveRBSRs", f, "UIDropDownMenuTemplate")
  ...
  RB.editHR = CreateFrame("EditBox", "RaidSlaveRBHR", f, "InputBoxTemplate")
  ...
  RB.editFree = CreateFrame("EditBox", "RaidSlaveRBFree", f, "InputBoxTemplate")
  ...
  RB.cbLFG = CreateFrame("CheckButton", "RaidSlaveRBLFG", f, "UICheckButtonTemplate")
  RB.cbLFG:SetWidth(20); RB.cbLFG:SetHeight(20); RB.cbLFG:SetPoint("LEFT", RB.cbWorld, "RIGHT", 40, 0)
  getglobal("RaidSlaveRBLFGText"):SetText("LFG")
  ...
  RB.cbYell = CreateFrame("CheckButton", "RaidSlaveRBYell", f, "UICheckButtonTemplate")
  RB.cbYell:SetWidth(20); RB.cbYell:SetHeight(20); RB.cbYell:SetPoint("LEFT", RB.cbLFG, "RIGHT", 30, 0)
  getglobal("RaidSlaveRBYellText"):SetText("Yell")
  ...
  RB.cbAuto = CreateFrame("CheckButton", "RaidSlaveRBAuto", f, "UICheckButtonTemplate")
  RB.cbAuto:SetWidth(20); RB.cbAuto:SetHeight(20); RB.cbAuto:SetPoint("LEFT", RB.cbYell, "RIGHT", 30, 0)
  getglobal("RaidSlaveRBAutoText"):SetText("Auto-Announce")
  RB.cbAuto:SetChecked(RB.state.auto); RB.cbAuto:SetScript("OnClick", OnAutoClick)

  RB.ddInterval = CreateFrame("Frame", "RaidSlaveRBInterval", f, "UIDropDownMenuTemplate")
...
-- Auto-Assign
RB.cbRoleAssign = CreateFrame("CheckButton", "RaidSlaveRBRoleAssign", f, "UICheckButtonTemplate")
RB.cbRoleAssign:SetWidth(20); RB.cbRoleAssign:SetHeight(20)
RB.cbRoleAssign:SetPoint("TOPLEFT", f, "TOPLEFT", 20, -340)
getglobal("RaidSlaveRBRoleAssignText"):SetText("Auto-Assign")
...
-- Auto-Invite
RB.cbAutoInvite = CreateFrame("CheckButton", "RaidSlaveRBAutoInvite", f, "UICheckButtonTemplate")
RB.cbAutoInvite:SetWidth(20); RB.cbAutoInvite:SetHeight(20)
RB.cbAutoInvite:SetPoint("LEFT", RB.cbRoleAssign, "RIGHT", 70, 0)
getglobal("RaidSlaveRBAutoInviteText"):SetText("Auto-Invite")
...
RB.cbAutoInvite:SetScript("OnClick", function()
  ...
  RB.ddGearScale = CreateFrame("Frame", "RaidSlaveRBGearScale", f, "UIDropDownMenuTemplate")
  ...
  RB.cbGear = CreateFrame("CheckButton", "RaidSlaveRBGear", f, "UICheckButtonTemplate")
...
RB.cbGear:SetScript("OnClick", function()
  ...
  RB.editSRLink = CreateFrame("EditBox", "RaidSlaveRBSRLink", f, "InputBoxTemplate")
  ...
  RB.editDiscordLink = CreateFrame("EditBox", "RaidSlaveRBDcLink", f, "InputBoxTemplate")
...
RaidSlaveDB.disablePopupThisRaid = RaidSlaveDB.disablePopupThisRaid or false
...
local function RB_ShowLeaderPopup()
    ...
    if RaidSlaveDB.disablePopupThisRaid then return end

    popupShownThisRaid = true

    local f = CreateFrame("Frame", "RaidSlaveLeaderPopup", UIParent)
    RB._leaderPopup = f
    ...
    local mlDD = CreateFrame("Frame", "RaidSlaveRBLeaderPopupML", f, "UIDropDownMenuTemplate")
    ...
    UIDropDownMenu_Initialize(mlDD, function()
      UIDropDownMenu_AddButton({
        ...
        func = function()
          ...
          if type(RaidSlaveRaidRoles_SetPresetMasterLooter) == "function" then
    ...
    local currentML = (type(RaidSlaveRaidRoles_GetPresetMasterLooter) == "function" and RaidSlaveRaidRoles_GetPresetMasterLooter()) or ""
    ...
    RaidSlaveDB.disablePopupThisRaid = this:GetChecked()
	end)
...
RB_leaderEvt:SetScript("OnEvent", function()
    ...
    if not inRaid and selfWasInRaid then
        RaidSlaveDB.disablePopupThisRaid = false
    ...
    if popupShownThisRaid or RaidSlaveDB.disablePopupThisRaid then return end
...
function RB_OpenExcludeList()
  ...
  local f = RB.frame
  local p = CreateFrame("Frame", "RaidSlaveRBExclude", f)
  RB.excl = p
  ...
  local scrollFrame = CreateFrame("ScrollFrame", "RaidSlaveRBExcludeScroll", p, "UIPanelScrollFrameTemplate")
  ...
  scrollFrame:SetScript("OnMouseWheel", function()
    local sb = getglobal("RaidSlaveRBExcludeScrollScrollBar")
